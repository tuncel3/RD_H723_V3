...
if (ms_50_cnt-gercek_voltaj_smp_ms_timer_h >= gercek_voltaj_smp_ms_timer_per && check_gercek_voltaj_stable_ms==1) {
	gercek_voltaj_diff_from_sec_ms=ABS(gercek_voltaj_smp_ms-VRECT_per_avg_roll64_sc);
...
//		sprintf(DUB,"diff > set val"); umsg(pr_btln, DUB);
	...
	gercek_voltaj_smp_ms=VRECT_per_avg_roll64_sc; // önceki sample
}
...
if (gercek_voltaj_stable_ms==1 && vout_sample_req==1 && vout_sample_ready==0) {
	...
	vout_sample_ready=1;
	baslangic_voltaj_ms=VRECT_per_avg_roll64_sc; // ismi baslangıc voltajı. yani inspection baslangıc voltajı.
	...
	if (ms_50_cnt-insp_loop_delay_h >= insp_loop_delay_per) {
		...
		if (batt_current_detected==0) { // batarya voltajı varsa direk batarya hattı sağlam denebilir. inspection devam etmemeli.
			if (vout_sample_ready==0) { // inspection başlama voltajını tutma işlmi tamam mı.
				...
				gercek_baslangic_fark=ABS(baslangic_voltaj_ms - VRECT_per_avg_roll64_sc); // voltaj kaydırma işlemi gerçekleştiriliyor.
				hedef_gercek_fark=ABS(V_targ_con_sy - VRECT_per_avg_roll64_sc);	// tutulan voltaj ile hedef ve gerçek voltaj arası farklar izleniyor.
...
if (fabs(IBAT_per_avg_roll16_sc) > batt_current_detect_threshold) {
...
} else if (fabs(IBAT_per_avg_roll16_sc) <= batt_current_detect_threshold) {
