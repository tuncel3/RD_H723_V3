...
void bup_fnc(void) {
	management_menu_en_cnt=0;
	management_menu_dis_cnt=0;
    if (currentPage == HOME_PAGE_pg) {															// UP
//    	asm("NOP");
    	print_active_states();
    }
...
if (!chg_setting_edit_mode) {
	...
	if (CHARGE_SETT_Items[selected_CHARGE_SETT].V1==SET_CHARGE_MODE) {
		EpD[SET_CHARGE_MODE][1].V1=((uint32_t)EpD[SET_CHARGE_MODE][1].V1-1+NUM_CH_MOD_SEL_ITEMS) % NUM_CH_MOD_SEL_ITEMS;
	} else if (CHARGE_SETT_Items[selected_CHARGE_SETT].V1==SET_CH_CONT_MODE) {
		EpD[SET_CH_CONT_MODE][1].V1=((uint32_t)EpD[SET_CH_CONT_MODE][1].V1-1+NUM_CH_CONT_SEL_ITEMS) % NUM_CH_CONT_SEL_ITEMS;
	} else if (CHARGE_SETT_Items[selected_CHARGE_SETT].V1==SET_BOOST_TIME) {
...
//				temp_test_thy_1 += test_step_values[test_dig];
        ...
        if (rel_disp_mode == 1) {
			if (sirali_tablo_arrow_ind > 1) {
				sirali_tablo_arrow_ind--;
			} else if (sirali_tablo_arrow_ind == 1) {
				sirali_tablo_disp_start_ind = (sirali_tablo_disp_start_ind - 1 + SIRALI_TABLO_SIZE) % SIRALI_TABLO_SIZE;
			}
			sirali_tablo_sel = (sirali_tablo_arrow_ind + sirali_tablo_disp_start_ind) % SIRALI_TABLO_SIZE;				// UP
...
//    	}
        ...
        if (cal_pg2_sel_edit_mode == cal_gain && cal_pg2_sel_col == 0) {
            if (cal_pg2_sel_digit == 0) {                                                // UP
                if      (cal_pg2_sel_item_left == 0) { EpD[SET_FRQ_CAL][0].V1 *= 1.00005; EpD[SET_FRQ_CAL][1].V1 = EpD[SET_FRQ_CAL][0].V1; frq_cal_k=275e6*EpD[SET_FRQ_CAL][0].V1;} // UP
            } else if (cal_pg2_sel_digit == 1) {                                         // UP
                if      (cal_pg2_sel_item_left == 0) { EpD[SET_FRQ_CAL][0].V1 *= 1.0002; EpD[SET_FRQ_CAL][1].V1 = EpD[SET_FRQ_CAL][0].V1; frq_cal_k=275e6*EpD[SET_FRQ_CAL][0].V1;} // UP
            }
        }
...
if (!chg_setting_edit_mode) {
	...
	if (CHARGE_SETT_Items[selected_CHARGE_SETT].V1==SET_CHARGE_MODE) {
        ...
        if (rel_disp_mode == 1) {
			if (sirali_tablo_arrow_ind < 4) {
				sirali_tablo_arrow_ind++;
			} else if (sirali_tablo_arrow_ind == 4) {
				sirali_tablo_disp_start_ind = (sirali_tablo_disp_start_ind + 1 + SIRALI_TABLO_SIZE) % SIRALI_TABLO_SIZE;	// DOWN
			}
			sirali_tablo_sel = (sirali_tablo_arrow_ind + sirali_tablo_disp_start_ind) % SIRALI_TABLO_SIZE;

        } else if (rel_edit_mode == 1) {

			if (tam_tablo_arrow_ind < 4) {
				tam_tablo_arrow_ind++;
			} else if (tam_tablo_arrow_ind == 4) {
				tam_tablo_disp_start_ind = (tam_tablo_disp_start_ind + 1 + TAM_TABLO_RELOUT_SIZE) % TAM_TABLO_RELOUT_SIZE;	// DOWN
			}
			tam_tablo_sel = (tam_tablo_arrow_ind + tam_tablo_disp_start_ind) % TAM_TABLO_RELOUT_SIZE;
//			if (tam_tablo_sel == tam_tablo_size-1) tam_tablo_sel=0;
...
//            }
        ...
        if (cal_pg2_sel_edit_mode == cal_gain && cal_pg2_sel_col == 0) {
            if (cal_pg2_sel_digit == 0) {                                                // DOWN
                if      (cal_pg2_sel_item_left == 0) { EpD[SET_FRQ_CAL][0].V1 /= 1.00005; EpD[SET_FRQ_CAL][1].V1 = EpD[SET_FRQ_CAL][0].V1; frq_cal_k=275e6*EpD[SET_FRQ_CAL][0].V1;} // DOWN
            } else if (cal_pg2_sel_digit == 1) {                                         // DOWN
                if      (cal_pg2_sel_item_left == 0) { EpD[SET_FRQ_CAL][0].V1 /= 1.0002; EpD[SET_FRQ_CAL][1].V1 = EpD[SET_FRQ_CAL][0].V1; frq_cal_k=275e6*EpD[SET_FRQ_CAL][0].V1;} // DOWN
            }
        }
        else if (cal_pg2_sel_edit_mode == cal_offset && cal_pg2_sel_col == 0) {
            if (cal_pg2_sel_digit == 0) {                                                // DOWN
