...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	inline extern void CALIBRATION_2_pg_disp(void) {
	    ...
	    sprintf(L, "VRc%6.2f", VRECT_pas.a64); 		GLCD_PrintString(0, 1*lnhg, L);
	    sprintf(L, "VL% 6.2f", VLOAD_pas.a64); 		GLCD_PrintString(0, 2*lnhg, L);
	    sprintf(L, "VB% 6.2f", VBAT_pas.a64); 		GLCD_PrintString(0, 3*lnhg, L);
...
} else if (state_list[SOFT_START_ST].action & (1 << ACTIVE_enum)) {
	sprintf(start_stop_str, ":%s", state_list[SOFT_START_ST].name); GLCD_PrintString(92, 0, start_stop_str);
}
	...
	for (tabl_dolas = son_kal; tabl_dolas < NUM_STATE_NAMES; tabl_dolas++) {
		if (state_list[tabl_dolas].action & (1 << LCD_roll_enum) && state_list[tabl_dolas].action & (1 << ACTIVE_enum)) {
			if (state_list[tabl_dolas].code==FLOAT_CHARGE_FC) {
				sprintf(RollBuf, "%s %s", state_list[tabl_dolas].name);
			}
