...
inline extern void RELAY_ORDER_pg_disp(void) {
    ...
    if (rel_disp_mode == 1) {
		...
		uint8_t sirali_tablo_index_=sirali_tablo_index; // aşağıdaki for döngüsü
		for (uint8_t i = 0; i < 6; i++) {

		    sprintf(L, " %02d", sirali_tablo_index_+1);
			GLCD_PrintString(0, (i + 1) * 9, L);
			sprintf(L, "%s ", state_list[(SIRALI_TABLO_RELOUT[sirali_tablo_index_].rel_out_code)].name);
			GLCD_PrintString(22, (i + 1) * 9, L);
			sprintf(L, "%d ", (state_get(SIRALI_TABLO_RELOUT[sirali_tablo_index_].rel_out_code)));
			GLCD_PrintString(122, (i + 1) * 9, L);

			sirali_tablo_index_=(sirali_tablo_index_+1+rel_ord_tb_size) % rel_ord_tb_size;
