...
//    Aşr Sıc Açma Süre  10 sn
		sprintf(L, " %s C  %5.0f", FANS_TEMP_Items[0], EpD[SET_COOL_FAN_TEMP][fan_temp_edit_mode].V1);	GLCD_PrintString(0, 1*lnhg, L);
		sprintf(L, " %s C    %5.0f", FANS_TEMP_Items[1], EpD[SET_TRANSF_FAN_TEMP][fan_temp_edit_mode].V1);	GLCD_PrintString(0, 2*lnhg, L);
		sprintf(L, " %s C%5.0f", FANS_TEMP_Items[2], EpD[SET_OVERTEMP_ALARM][fan_temp_edit_mode].V1);	GLCD_PrintString(0, 3*lnhg, L);
		sprintf(L, " %s C %5.0f", FANS_TEMP_Items[3], EpD[SET_OVERTEMP_OPEN][fan_temp_edit_mode].V1);	GLCD_PrintString(0, 4*lnhg, L);
		sprintf(L, " %s     %03lu", FANS_TEMP_Items[4], (uint32_t)EpD[SET_OVT_OPEN_DELAY][fan_temp_edit_mode].V1);	GLCD_PrintString(0, 5*lnhg, L);
}
inline extern void DROPPER_pg_disp(void) {
	...
	if (!dropper_edit_mode) {
		...
		sprintf(L, " Üst Lm %5.1fV %4.1f%%", set_dropper_l_hg_V_h, set_dropper_l_hg_perc_h); 	GLCD_PrintString(0, 4*lnhg, L);
		sprintf(L, " Alt Lm %5.1fV %4.1f%%", set_dropper_l_lw_V_h, set_dropper_l_lw_perc_h); 	GLCD_PrintString(0, 5*lnhg, L);
	}
