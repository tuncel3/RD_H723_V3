...
inline extern void RELAY_ORDER_pg_disp(void) {
    ...
    if (rel_disp_mode == 1) {
		...
		sprintf(L, "Kontak %02d İçin Seçim", SIRALI_TABLO_RELOUT[sirali_tablo_disp_start_ind].tbl_sira);
		...
		for (uint8_t i = 0; i < 6; i++) {
			...
			sprintf(L, "%d ", TAM_TABLO_RELOUT[tam_tablo_disp_start_ind_].tbl_val);
			GLCD_PrintString(122, (i + 1) * 9, L);
